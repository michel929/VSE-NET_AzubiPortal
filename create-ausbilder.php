...
if(isset($_POST["vorname"])) {
    ...
    if($beruf == "Neue Abteilung"){
        if(empty($_POST["nameabteilung"]) || empty($_POST["colorabteilung"])){
            $errormsg = '<div class="alert alert-custom alert-indicator-top indicator-danger" role="alert">
    <div class="alert-content">
        <span class="alert-title">Fehler beim anlegen des Azubis</span>
        <span class="alert-text">Du musst alle Felder ausfüllen.</span>
    </div>
</div>';
        }else {
            //TODO INsert Abteilung
        }
    }
    ...
    if (!$error) {
        ...
        if ($users !== false) {
            $errormsg = '<div class="alert alert-custom alert-indicator-top indicator-danger" role="alert">
    <div class="alert-content">
        <span class="alert-title">Fehler beim anlegen des Ausbilders</span>
    ...
    if (!$error) {
        ...
        $statement = $pdo->prepare("INSERT INTO users (email, passwort, vorname, nachname, rolle, abteilung) VALUES (:email, :passwort, :vorname, :nachname, :rolle, :abteilung)");
        $result = $statement->execute(array('email' => $email1, 'passwort' => $passwort_hash, 'vorname' => $vorname1, 'nachname' => $nachname1, 'rolle' => $rolle, 'abteilung' => $beruf));
                            ...
                            <form class="row g-3" method="post">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select" onchange="change();" id="select" aria-label="Default select example">
                                        <option selected>Wähle eine Abteilung</option>
                                        <?php
                                ...
                                <div class="col-md-6" id="abteilungenName">
                                    <label for="inputPassword4" class="form-label">Abteilungsnamen</label>
                                    <input type="text" name="nameabteilung" class="form-control" id="vorname" onchange="onText()" required>
                                </div>
                                <div class="col-md-6" id="abteilungenColor">
                                    <label for="inputPassword4" class="form-label">Color</label>
                                    <input type="color" name="colorabteilung" class="form-control" id="vorname" onchange="onText()" required>
                                </div>

    <div class="col-12">
        <button type="submit" class="btn btn-primary">Ausbilder anlegen</button>
    </div>
</form>
                        </div>
                    </div>
...
<script>
    function ready(){
        document.getElementById("abteilungenName").style.display = "none";
        document.getElementById("abteilungenColor").style.display = "none";
    }

    function change(){

        if($('#select').val() == "new"){
            document.getElementById("abteilungenName").style.display = "block";
            document.getElementById("abteilungenColor").style.display = "block";
        }else {
            document.getElementById("abteilungenName").style.display = "none";
            document.getElementById("abteilungenColor").style.display = "none";
        }
